...
func save_translations_po(path:String)->void :
		...
		while not file.eof_reached():
			...
			if line.begins_with("msgid"):
				...
				if msgid in id_str:
					...
					while not file.eof_reached() and not line.empty() and (line.begins_with("msgstr") or line.begins_with("\"")):
						line = file.get_line().strip_edges()
					existing_po += line + "\n"
				else :
					existing_po += line + "\n"
	...
	if not ("" in already_existing_keys):
		existing_po += generate_po_line("msgid", "")
		existing_po += "msgstr \"\"\n\"Content-Type: text/plain; charset=UTF-8\\n\"" + "\n" + "\n"

	for key in id_str:
		if not (key in already_existing_keys):
